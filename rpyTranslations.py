...
def check(forFiles, /, untranslated=1, formats=False, *, verbose=0, debug=False):
	...
	if not untranslated in (0,1,2):
		Throw(ValueError,"Invalide argument: untranslated parameter should be 1 for empty, 2 for pass, or 0 to deactivate", code=2)
	if not isinstance(formats, bool):
		Throw(TypeError,"Invalide argument: formats parameter should be of booleen type", code=2)
	if not formats and untranslated == 0:
		Throw(Exception,"Invalide parameters: formats parameter need to be set to True or untranslated parameter need to be greater than 0 to indicate which checking to make", code=3)
...
def fixEmpty(forFiles, /, action='P', *, outdir=None, verbose=0, debug=False):
	...
	if not action in ('P','C','R'):
		Throw(ValueError,"Invalide argument: action parameter should be P, C, or R", code=2)
	popFiles = []
	for forF in forFiles:
		if not os.path.isfile(forF):
			Throw(FileNotFoundError,repr(forF),"doen't exist or is not a file")
		if not outdir is None:
			...
			if pF in popFiles: Throw(Exception,"All files need to be different")
			popFiles.append(os.path.join(fPath[0],outdir,fPath[1]))
		else:
			if forF in popFiles: Throw(Exception,"All files need to be different")
...
def reorder(forFiles, /,*, reverse=False, proxy=False, outdir=None, verbose=0, debug=False):
	...
	proxy: Booleen - Active or not the regroupement function of their normalized alpha-numeric representation.
	outdir: String - A path relative from the 'forFiles' paths to indicate where to save the result or None
	        to save inplace.
	verbose: Integer - Indicate the level of verbosity, 0 to deactivate.
	...
	if not isinstance(reverse, bool):
		Throw(TypeError,"Invalide argument: reverse parameter should be of booleen type", code=2)
	if not isinstance(proxy, bool):
		Throw(TypeError,"Invalide argument: proxy parameter should be of booleen type", code=2)
	popFiles = []
	for forF in forFiles:
		if not os.path.isfile(forF):
			Throw(FileNotFoundError,repr(forF),"doen't exist or is not a file")
...
def populate(forFiles, *FromFiles, proxy=0, overwrite='N', outdir=None, verbose=0, debug=False):
	...
	if not proxy in (0,1,2,3):
		Throw(ValueError,"Invalide argument: proxy should be an integer between 0 and 3 (includes)", code=2)
	if not overwrite in ('N','A','F'):
		Throw(ValueError,"Invalide argument: overwrite parameter should be N for no, A for ask, or F for force", code=2)
	popFiles = []
			...
			while not (M_string is None and M_dialog is None):
				...
				if not M_from is None:
					...
					if (not by_proxy and not Pass and not M.group('new_str') in ('""', M_from.group('new_str')) and overwrite == 'A') or (by_proxy and (Pass or M.group('new_str') == '""')):
						print(f"For: {M.group('old_str')}\nAttempt to replace: {M.group('new_str') if not Pass else 'pass'}\nFrom: {M_from.group('new_str')}")
						if by_proxy: print("Warning: Please note that the 'From' was found by proxy.")
						while not A:
							A = input("Proceed? (N|Y) >>> ")
...
if __name__ == '__main__':
	...
	if Cmd in ('-h','--help'):
		print(dedent(f"""\
		python3 -m rpyTranslations ({'|'.join(Cmds)}) [args] [options]
		...
		options
		  Their generaly can be put everywhere in the argument realm of command.
		...
		"""))
		if len(sys.argv) != 1: Error("Invalide argument: help commands do not accept any argument or option.")
		exit()
		...
		options:
		  ...
		  -o dir, --subdir dir
		      If puts, it indicate a sub-directory to where the populated translation files are saved,
		  ...
		  -a, --ask
		      If puts, a prompt will ask to confirm or no a replacement of translation (only for non-empty).
		  -f, --force, --overwrite
		...
		* Please note that --ask take precedence over --force.
		...
		options:
		  ...
		  -o dir, --subdir dir
		      If puts, it indicate a sub-directory to where the fixed translation files are saved, otherwise
		...
		options:
		  ...
		  -e, --empty
		      Default. Check how mani translation is still empty.
		  -p, --not-translate
		      Check how mani translation is still empty or are set with the 'pass' keyword.
		  -f, --format
		...
		* Please note that --skip-empty take precedence over --not-translate that take precedence over --empty.
		...
		options:
		  ...
		  -o dir, --subdir dir
		      If puts, it indicate a sub-directory to where the reordered translation files are saved,
		       otherwise on the for-files themself.
		  -r, --reverse
		      If puts, try to reordering as it was originaly (as extracted by Ren'Py) through the
